...
async function broadcastSendWithClient(client, userId, message, mediaBase64, mediaMime, mediaType, videoRound) {
  ...
  if (mediaBase64 && mediaType) {
    ...
    // Salva em arquivo temporário para o GramJS ler
    const tmpFile = path.join(os.tmpdir(), `bc_media_${Date.now()}`);

    if (mediaType === 'image') {
      fs.writeFileSync(tmpFile + '.jpg', buf);
      await client.sendFile(targetPeer, {
        file: tmpFile + '.jpg',
        caption: message || '',
        forceDocument: false
      });
      try { fs.unlinkSync(tmpFile + '.jpg'); } catch(e) {}
      ...
      fs.writeFileSync(tmpFile + '.ogg', buf);
      await client.sendFile(targetPeer, {
        file: tmpFile + '.ogg',
        caption: '',
        voiceNote: true,
      ...
      try { fs.unlinkSync(tmpFile + '.ogg'); } catch(e) {}
      ...
      fs.writeFileSync(tmpFile + '.mp4', buf);
      await client.sendFile(targetPeer, {
        file: tmpFile + '.mp4',
        caption: '',
        videoNote: true,
      ...
      try { fs.unlinkSync(tmpFile + '.mp4'); } catch(e) {}
      if (message) await client.sendMessage(targetPeer, { message });
...
app.post('/api/broadcast/start', authMiddleware, async (req, res) => {
  ...
  console.log(`[BROADCAST/START] message="${message?.substring(0,30)}" aiMode=${!!aiObjective} mediaType=${mediaType} hasMedia=${!!mediaBase64}`);
...
// ── Gerar mensagem IA para um lead específico ──
async function generateAiMessageForLead(objective, tone, name) {
  const toneMap = {
    casual: 'casual e amigável',
    urgente: 'urgência e escassez',
    empolgado: 'muito empolgado com emojis',
    curiosidade: 'gerando curiosidade',
...
Responda APENAS com um JSON assim, sem markdown, sem explicações:
